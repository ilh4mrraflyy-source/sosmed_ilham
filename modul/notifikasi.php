...
<body>
            ...
            <ul class="navbar-nav ms-auto align-items-center">
                <li class="nav-item position-relative me-3">
                    <a href="#" class="nav-link text-white position-relative" id="notifIcon">
                        ...
                        <?php if ($count > 0): ?>
                        <span
                            class="position-absolute top-0 start-100 translate-middle badge rounded-pill bg-danger"><?= $count ?></span>
                        <?php endif; ?>
                    </a>
                </li>
                <li class="nav-item">
    ...
    <div class="notif-container">
        ...
        // Status untuk permintaan teman
        if ($n['Type'] == "friend_request") {
          $req = mysqli_query($conn, "SELECT Status FROM friend_requests WHERE FromUser='$from' AND ToUser='$username'");
          if ($req && mysqli_num_rows($req) > 0) {
            $r = mysqli_fetch_assoc($req);
            if ($r['Status'] == 'accepted') $status = '<span class="status-label status-accepted">✅ Sudah Berteman</span>';
            if ($r['Status'] == 'rejected') $status = '<span class="status-label status-rejected">❌ Ditolak</span>';
          }
        }
      ?>
        <div class="notif-item <?= $n['IsRead'] ? '' : 'unread' ?>">
            ...
            <div class="notif-text">
                ...
                <?php if ($n['Type'] == "friend_request" && empty($status)): ?>
