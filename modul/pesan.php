...
<head>
  ...
  <style>
    body { background-color: #f0f2f5; font-family: "Segoe UI", Arial, sans-serif; overflow: hidden; }
    ...
    .friend-list { width: 28%; background-color: #fff; border-right: 1px solid #ddd; overflow-y: auto; }
    .friend-item { display: flex; align-items: center; padding: 12px 15px; border-bottom: 1px solid #eee; text-decoration: none; color: #050505; position: relative; }
    .friend-item:hover { background-color: #f0f2f5; }
    .friend-item img { width: 45px; height: 45px; border-radius: 50%; margin-right: 10px; }
    .online-dot { position: absolute; left: 50px; bottom: 12px; width: 10px; height: 10px; background: #31a24c; border-radius: 50%; border: 2px solid white; }
    .search-box { padding: 10px; border-bottom: 1px solid #ddd; }
    .search-box input { width: 100%; border-radius: 20px; border: none; padding: 8px 15px; background: #f0f2f5; }
    .chat-area { flex: 1; display: flex; flex-direction: column; background-color: #e5ddd5; position: relative; }
    .chat-header { background-color: #fff; padding: 15px; border-bottom: 1px solid #ddd; font-weight: 600; }
    .chat-messages { flex: 1; padding: 15px; overflow-y: auto; display: flex; flex-direction: column; }
    .msg { max-width: 70%; padding: 10px 15px; margin-bottom: 10px; border-radius: 18px; font-size: 14px; }
    .msg.me { background-color: #1877f2; color: white; align-self: flex-end; border-bottom-right-radius: 4px; }
    .msg.them { background-color: #f0f0f0; color: #050505; align-self: flex-start; border-bottom-left-radius: 4px; }
    .chat-input { background-color: #fff; padding: 10px 15px; display: flex; border-top: 1px solid #ddd; }
    .chat-input input { flex: 1; border: none; border-radius: 20px; padding: 10px 15px; background-color: #f0f2f5; }
    .chat-input button { border: none; background-color: #1877f2; color: white; border-radius: 50%; width: 40px; height: 40px; margin-left: 10px; font-weight: bold; }
    .no-chat { display: flex; align-items: center; justify-content: center; color: #888; font-size: 18px; flex: 1; }
  </style>
</head>
<body>
  ...
  <script>
    ...
    form.addEventListener('submit', e => {
      ...
      fetch('pesan.php', { method: 'POST', body: formData });
      pesanInput.value = '';
